    ...
    <section className="mx-auto w-full max-w-5xl rounded-3xl border border-white/10 bg-slate-900/30 px-6 py-10 shadow-2xl backdrop-blur-md">
      ...
      <div className="mt-8 pt-8 border-t border-white/10">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {contacts.map((item) => {
            // Open mailto links in same tab, others in new tab, phone does nothing
            ...
            if (isPhone) {
              return (
                <div
                  key={item.label}
                  className="flex items-center gap-3 rounded-2xl border border-white/15 bg-slate-900/30 px-5 py-3 text-amber-100"
                >
